...
dados_corpo = PyPDF2.PdfReader(pdf_corpo)
numero_pag_corpo=len(dados_corpo.pages)           # Contando o número de páginas do arquivo

if numero_pag_corpo == 1:                       # Lendo arquivo com 1 página
    pag_corpo = dados_corpo.pages[0]
    txt_corpo = pag_corpo.extract_text()
    txt_corpo = ''.join(txt_corpo).lower()
    txt_corpo = re.sub('\n', '', txt_corpo)
else:
    i = 1
    while (i <= numero_pag_corpo):                  # Lendo arquivo com mais de uma página
        pag_corpo = dados_corpo.pages[i - 1]
        txt_corpo = pag_corpo.extract_text()
...
for att in attachments:
    ...
    if file_extension == '.pdf':
        ...
        dados_anexo = PyPDF2.PdfReader(pdf_file)
